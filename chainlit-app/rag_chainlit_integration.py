...
- **Processing Time:** {processing_time:.2f}s
    ...
    async def expand_query(self, query: str) -> List[str]:
        ...
        alternatives = await self.rag_processor.pipeline.query_expansion.generate_alternatives_async(query)
...
**Document Length:** {len(hyde_doc)} characters
    ...
    async def perform_retrieval(self, queries: List[str], hyde_doc: str, retrieval_k: int = 20) -> List[Any]:
        """Step 4: Perform retrieval for all queries"""
        current_step = cl.context.current_step
        current_step.input = f"Queries: {len(queries)}, HyDE Doc: {len(hyde_doc)} chars, K: {retrieval_k}"
        ...
        current_step.output = f"""
**Retrieval Results:**
- **Queries Processed:** {len(queries) + 1}
- **Total Documents Retrieved:** {total_documents}
- **Average Documents per Query:** {total_documents / (len(queries) + 1):.1f}
- **Processing Time:** {processing_time:.2f}s
...
{chr(10).join([f"• Query {i+1}: {len(results)} documents" for i, results in enumerate(retrieval_results)])}
    ...
    async def apply_rrf_fusion(self, retrieval_results: List[Any]) -> List[Any]:
        ...
        fused_results = self.rag_processor.apply_rrf_fusion_async(retrieval_results)
...
- **Fusion Method:** Reciprocal Rank Fusion (k=60)
    ...
    async def apply_cohere_reranking(self, query: str, documents: List[Any]) -> List[Any]:
        ...
        reranked_docs = self.rag_processor.apply_cohere_reranking_async(query, documents)
    ...
    async def process_complete_pipeline(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            retrieval_results = await self.perform_retrieval(
                alternative_queries, hyde_document
            )
